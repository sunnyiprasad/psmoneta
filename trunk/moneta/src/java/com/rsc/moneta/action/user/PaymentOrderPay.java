 ...
 * @author sulic
 */
public class PaymentOrderPay extends BaseAction {
        ...
        if (accountId == null) {
            ...
            if (account == null) {
                ...
                if (account.getBalance() < paymentOrder.getAmount()) {
                    addActionError("you_hasnt_money");
                    return Action.ERROR;
                } else {
                    ...
                    if (response == null) {
                        ...
                        if (response.getResultCode() == ResultCode.SUCCESS_WITH_AMOUNT
                            ...
                            response = handler.pay(paymentOrder, 0);
                            if (response == null) {
                                addActionError(getText("system_error_connect_with_admin"));
                                return Action.ERROR;
                            } else {
                                ...
                                } else if (response.getResultCode() == ResultCode.SUCCESS_BUT_AMOUNT_LESS_THAN_MUST_BE) {
                                    addActionError("Произошла непредвиденная ошибка системы, срочно свяжитесь администратором системы");
                                    return Action.ERROR;
                                } else {
